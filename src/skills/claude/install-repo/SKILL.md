...
Also invoke when the user asks Claude to find, install, or remove a GitHub project by name, URL, owner/repo, username, natural-language description, or commit ref.
...
| Search query       | `a skill that lets Claude install GitHub repos`  |
...
The CLI does **not** execute README content. Claude must not infer destructive shell commands from repo documentation and run them without showing the user the exact commands first.
